...
### What do we look for in a plot? [https://dub.sh/biosci220](dub.sh/biosci220)
...
p1 <- ggplot(penguins_nafree,aes(x = species, y = bill_length_mm)) + 
    geom_violin()  + 
  ylab("Bill length (mm)") +
  xlab("") +
  ...
  labs(subtitle = "Between group variation")
jit <- ggplot() + 
...
p2 <- ggplot() + 
  ...
  labs(subtitle = "Within group variation")
p1 + p2 
```
...
## [live demo] - come and collect your *experiment pack*

![](img/sweet_sorting-01.png)

---
class: center, middle, inverse
 
## [live demo] - Link to record your data
 
## [LINK - Data recording sheet](https://docs.google.com/spreadsheets/d/1njyRAzRFg-IKXXG-BUOCXfUyyC6mRPrz1OnQ4Hji2E8/edit?usp=sharing) 
(*need to be signed into your UoA account*)
...
[`R` demo] - download [sweets.csv](https://canvas.auckland.ac.nz/courses/120675/files/15934050?wrap=1) from CANVAS (i.e., last lectures' data)

```{r, eval = FALSE}
library(tidyverse)
sweets <- read_csv("sweets.csv")
## drop NA Time (response) values
sweets <- sweets[!is.na(sweets$Time),] 
## One unusually high time value 
## here I've chosen to assume the , is meant to be a decimal 
## and have overwritten it 'manually' (is this best practice?)
sweets$Time[1] <- 49.53
## Plotting 
ggplot(sweets, aes(x = Treatment, y = Time)) +
  geom_violin()
...
**Are the means of each treatment are *significantly* different from each other? Why or why not?**<p><a href="https://dub.sh/biosci220" target="_blank">dub.sh/biosci220</a></p>

```{r, eval = TRUE, echo = FALSE,message = FALSE}
library(tidyverse)
data <- read_csv("https://raw.githubusercontent.com/STATS-UOA/databunker/master/data/sweets.csv")
meds <- data %>%
	group_by(Treatment) %>%
	summarise(means = round(mean(Time), 3))
ggplot(data, aes(x = Treatment, y = Time, col = Treatment)) + 
  geom_violin() + geom_jitter(aes(col = Treatment), size = 3) +
	xlab("Group") + ylab("Time to complete task (s)") + theme_light() +
	annotate(geom = "text", x = meds$Treatment, y = meds$means, 
	         label = paste("mu == ", meds$means), parse = TRUE,
	         col = c("#80CFF0", "#DFC0D0"), size = 10)  +
...
Let $\mu_\text{A}$ be the mean of *Treatment A* and $\mu_\text{B}$ be the mean of *Treatment B* 
...
diff_in_means <- data %>% 
  ...
  summarise(diff = diff(rev(mean))) %>% as.numeric()
round(diff_in_means, 3)
```

That is, $\mu_\text{A} - \mu_\text{B} = 1.494$ (on average a ~1.5(s) difference in time to complete the task)
---
class: center, middle, inverse

### [**Sticker Demo**] (*White = Treatment A, Blue = Treatment B*)

(NULL hypothesis) $H_0: \mu_\text{A} = \mu_\text{B}$ 

vs 

(alternative hypothesis) $H_1: \mu_\text{A} \neq \mu_\text{B}$

Take a sticker and...[update this sheet after following Charlotte's instructions](https://docs.google.com/spreadsheets/d/1xyda-lzAlPUWLzf2M2jNpOUWbaL38LX2XFxo4pGDMao/edit?usp=sharing) 
...
```{r, eval = FALSE}
library(tidyverse)
read_csv("https://raw.githubusercontent.com/STATS-UOA/databunker/master/data/sweets.csv") %>%
  mutate(random_labels  = sample(Treatment, replace = FALSE)) %>%
...
**Randomization test**
 ...
 + **Decide** on a metric (Note this metric can be **anything** you wish) to measure the effect in question, we used $\mu_\text{A} - \mu_\text{B}$
 
--
 
 + **Calculate** that test statistic on the observed data, we had $\mu_\text{A} - \mu_\text{B} = 1.494$
...
Let's go back to our in-class randomization test; how likely was what we observed, $\mu_\text{A} - \mu_\text{B} = 1.494$, to have occurred by chance alone? Use `R` to reshuffle our data labels 1000 times:
...
How likely was what we observed, $\mu_\text{A} - \mu_\text{B} = 1.494$, to occur by chance alone? A look at the full sampling distribution.

```{r, eval = FALSE}
ggplot(results, aes(x = randomisation_difference_mean)) +
  geom_histogram(bins = 35) +
  theme_bw() + theme(axis.text = element_text(size = 14)) + 
  ylab("Count") + xlab("Differences between randomised treatment means") +
...
How likely was what we observed, $\mu_\text{A} - \mu_\text{B} = 1.494$, to occur by chance alone? A look at the numbers.
...
That is, out of the 1000 iterations
 ...
 + a value >= `r round(diff_in_means,3)` was obtained `r  sum(results$randomisation_difference_mean >= diff_in_means)` times, and 
 + a value <= `r round(-1*diff_in_means,3 )` was obtained `r  sum(results$randomisation_difference_mean <= -1*diff_in_means)` times.
...
So, we have **no evidence against the NULL hypothesis** that $\mu_\text{A} = \mu_\text{B}$ (i.e., the difference between averages of time to complete tasks likely to have occurred by chance alone).
